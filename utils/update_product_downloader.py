...
def run_product_downloader(scan_path, parallel_jobs, spider_name, class_name):
    ...
    # Load spider class
    mod = __import__('updated_spiders.{}'.format(spider_name), fromlist=[class_name])
    spider_class = getattr(mod, class_name)
    update_products = spider_class()
    ...
    while True:
        ...
        proc = Process(target=do_downloading_job, args=(filename, update_products, downloaded_size_counter, total_size, process_id, idle_processes))
        procs.append(proc)
        proc.start()
...
def do_downloading_job(filename, update_products, downloaded_size_counter, total_size, process_id, finished_processes_queue):
    print("Downloading", filename)

    start_time = time.time()
    output_path = filename[0:-3] + "json"
    download_products(filename, output_path, update_products)
...
def download_products(source, output_path, update_products):
    # Load url list
    try:
        product_links = []
    ...
    crawl(product_links, update_products, process_output_path)
    ...
    # Compress to byte the string and Post to server
    compress_to_byte(output_path)

# Save to zip
def save_to_zip(output_path):
    if output_path.endswith('.json'):
        ...
        try:
            with open(output_path, encoding='utf-8') as f:
                data = f.read()
            output_file_name = output_path.replace('\\', '/')
            output_file_name = output_file_name.split('/')[-1]
            zf.writestr(output_file_name, data)
            zf.close()
        except Exception as e:
            print('Error when zip the file', e)
    return

# Compress to byte from string and post to server
def compress_to_byte(output_path):
    headers = {
    ...
    try:
        ...
        res = requests.post(url='http://', data=compressed_data, headers=headers)
    except Exception as e:
        print('Error when compress to byte the file', e)
    return None

def crawl(urls, update_products, output_path):
    values = update_products.parse_spiders(urls)
